...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    try { await api.post("register/", { username, password, email }); navigate("/"); }
    catch { alert("No se pudo registrar. El usuario puede ya existir."); }
  };
        ...
        <div className="bg-card dark:bg-dark-card border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-8 w-full max-w-sm">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              ...
              <input className={inputCls} placeholder="Nombre de usuario" value={username} onChange={e => setUser(e.target.value)} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Correo electrónico</label>
              <input type="email" className={inputCls} placeholder="ejemplo@alcaldia.gov.co" value={email} onChange={e => setEmail(e.target.value)} />
            </div>
            <div>
          ...
          <p className="mt-5 text-center text-sm text-gray-500 dark:text-gray-400">
            ¿Ya tienes cuenta?{" "}
            <Link to="/" className="font-semibold text-primary dark:text-primary-light hover:underline">Iniciar sesión</Link>
          </p>
        </div>
      </div>
    </div>
